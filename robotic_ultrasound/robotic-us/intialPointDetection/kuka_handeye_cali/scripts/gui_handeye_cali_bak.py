...
class CalibGUI(QMainWindow):
    ...
    def __init__(self, 
        ...
        self.cimg_sub = rospy.Subscriber(vis_img_topic_name, Image, self.sub_img_cb)
        rospy.wait_for_message('/camera/color/image_raw', Image, timeout=3)
        #  -- Joint states subscribe: "/franka_state_controller/joint_states"
        self.jpos_sub = rospy.Subscriber("/iiwa/state/CartesianPose", JointState, self.sub_jpos_cb)
        ...
        if not self.aruco_res is None:
            if self.aruco_res.__contains__(self.aruco_cali_id):
                ...
                for p in self.sampler.ar_trans: # trans
                    p = round(p, 2)
                    astr += '\t' + str(p)
                for p in self.sampler.ar_quat: # quatmodulenotfounderromodulenotfounderrorr
                    p = round(p, 3) 
                    astr += '\t' + str(p)
