...
class CalibGUI(QMainWindow):
    '''
    Description: 
        CalibGUI serves as a handeye calibration manager, that assembles
        the samples by processing messages from ros, and utilizes the 
        hand eye calibration api in cv2 to get the calibration result. The 
    ...
    def __init__(self, 
        ...
        # #  -- Joint states subscribe: /franka_state_controller/joint_states
        # self.jpos_sub = rospy.Subscriber("/iiwa/state/CartesianPose", 
        #                                 JointState, self.sub_jpos_cb)
        # rospy.wait_for_message("/iiwa/state/CartesianPose", JointState, timeout=3)
    ...
    def show_aruco_res(self, use_charuco=True):
    ...
    def initUI(self):
        ...
        self.setWindowTitle('Kuka Handeye Calibration Log cmd GUI')
        self.show()
...
if  __name__ == '__main__':
    ...
    gui = CalibGUI(
        sample_path=config_path+sample_filename,
