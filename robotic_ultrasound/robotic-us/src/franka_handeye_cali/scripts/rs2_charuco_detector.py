...
LastEditTime: 2022-11-12 15:19:02
Description: 
'''
# ros
import rospy
...
class Rs2ArucoDetector:
    ...
    def detect_aruco(self, cv2_img):
        ...
        # aruco_dict = aruco.Dictionary_get(aruco.DICT_ARUCO_ORIGINAL)  # old version
        aruco_dict = aruco.getPredefinedDictionary(aruco.DICT_ARUCO_ORIGINAL)
        # detector parameters can be set here (List of detection parameters[3])
        # parameters = aruco.DetectorParameters_create()  # old version
    ...
    def detect_charuco(self, cv2_img):
        ...
        corner_criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 100, 1e-5)
        gray = cv2.cvtColor(cv2_img, cv2.COLOR_BGR2GRAY)
        aruco_dict = aruco.getPredefinedDictionary(aruco.DICT_4X4_100)
        parameters = aruco.DetectorParameters()
        # constant to be passed into Aruco methods
        charubo_board = aruco.CharucoBoard(
        ...
        if ids is not None:
            ...
            retval, charucoCorners, charucoIds = aruco.interpolateCornersCharuco(markerCorners=corners,
                                                                                 markerIds=ids,
                                                                                 image=gray,
                                                                                 board=charubo_board)
            ...
            if charucoIds is not None:
                ...
                valid, rvec, tvec = aruco.estimatePoseCharucoBoard(charucoCorners=charucoCorners, charucoIds=charucoIds, board=charubo_board,
